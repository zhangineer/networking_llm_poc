    ...
    def run(self):
        while not self._exit:
            ...
            try:
                ...
                st.session_state.connection_error = "Could not login to APIC due to ConnectionError"
...
class Session(object):
    ...
    def _send_login(self, timeout=None):
        ...
        if self.appcenter_user and self._subscription_enabled:
            ...
            data = {'aaaUser': {'attributes': {'name': self.uid,
                                               'pwd': self.pwd}}}
        ret = self.push_to_device(url=login_url, data=data, timeout=timeout)
        if not ret.ok:
            ...
            print(json.loads(ret.text)['imdata'][0]['error']['attributes']['text'])
    ...
    def push_to_device(self, data, url='/ins', timeout=None, use_cli=True):
        """
        ...
        :use_cli: Whether to use CLI API, which uses ins API
        """
        ...
        if self.cert_auth and not (self.appcenter_user and self._subscription_enabled and self._logged_in):
            ...
            if use_cli:
                resp = self.session.post(post_url, data=json.dumps(data, sort_keys=True), verify=self.verify_ssl,
                                         timeout=timeout, proxies=self._proxies, auth=(self.uid, self.pwd))
            else:# if using DME model
                resp = self.session.post(post_url, data=json.dumps(data, sort_keys=True), verify=self.verify_ssl,
    ...
    def get(self, url, timeout=None):
        """
        Perform a REST GET call to the Nexus device.
